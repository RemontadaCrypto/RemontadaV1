...
class MailController extends Controller
{
    ...
    {
        $line1 = 'Your withdrawal of <b>'.number_format($payload['amount'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> to <b>'.$payload['party'].'</b> was successful';
    ...
    {
        $line1 = 'You have a new trade request of <b>'.number_format($payload['amount_in_coin'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> from <b>'.$payload['buyer']['name'].'</b> at <b>₦'.number_format($payload['amount_in_ngn']).'</b> with rate of <b>₦'.$payload['offer']['rate'].'</b> per USD, kindly attend to this swiftly.';
    ...
    {
        $line1 = 'Your trade of <b>'.number_format($payload['amount_in_coin'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> with <b>'.$payload['seller']['name'].'</b> at <b>₦'.number_format($payload['amount_in_ngn']).'</b> with rate of <b>₦'.$payload['offer']['rate'].'</b> per USD has been cancelled.';
    ...
    {
        $line1 = 'Your trade of <b>'.number_format($payload['amount_in_coin'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> with <b>'.$payload['buyer']['name'].'</b> at <b>₦'.number_format($payload['amount_in_ngn']).'</b> with rate of <b>₦'.$payload['offer']['rate'].'</b> per USD has been cancelled.';
        $user->notify(new CustomEmailNotification('Trade #'.$payload['ref'].' Cancelled', $line1));
    }

    public static function sendBuyerTransactionMail($user, $payload)
    {
        $line1 = 'Your have successfully bought <b>'.number_format($payload['amount_in_coin'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> from <b>'.$payload['seller']['name'].'</b> at <b>₦'.number_format($payload['amount_in_ngn']).'</b> with rate of <b>₦'.$payload['offer']['rate'].'</b> per USD';
        $user->notify(new CustomEmailNotification('Trade #'.$payload['ref'].' Successful', $line1));
    }

    public static function sendSellerTransactionMail($user, $payload)
    {
        $line1 = 'Your have successfully sold <b>'.number_format($payload['amount_in_coin'], 8).' '.strtoupper($payload['coin']['short_name']).'</b> to <b>'.$payload['buyer']['name'].'</b> at <b>₦'.number_format($payload['amount_in_ngn']).'</b> with rate of <b>₦'.$payload['offer']['rate'].'</b> per USD';
        $user->notify(new CustomEmailNotification('Trade #'.$payload['ref'].' Successful', $line1));
    }
}
